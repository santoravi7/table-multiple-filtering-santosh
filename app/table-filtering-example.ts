...
const ELEMENT_DATA: PeriodicElement[] = [{"id":1,"first_name":"Salvatore","last_name":"Loos","email":"sloos0@surveymonkey.com","gender":"Male","ip_address":"171.233.241.55","timestamp":"1554052388"},
...
export class TableFilteringExample {
  ...
  constructor(formBuilder: FormBuilder) {
    this.dataSource.filterPredicate = ((data, filter) => {
      ...
      const b =
        !filter.first_name ||
        data.first_name.toLowerCase().includes(filter.first_name.toLowerCase());
      const c =
        !filter.last_name ||
        data.last_name.toLowerCase().includes(filter.last_name.toLowerCase());
      const d = !filter.email || data.email.includes(filter.email);
      const e = !filter.gender || data.gender.includes(filter.gender);
      const f =
        !filter.ip_address || data.ip_address.includes(filter.ip_address);
      const g = !filter.timestamp || data.timestamp.includes(filter.timestamp);
